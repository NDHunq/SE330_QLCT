// src/container/user.container.ts
...
import { OtpTokenRepository } from "@/repository/otp-token.repository"; // Thêm import
...
userContainer
  .bind<OtpTokenRepository>(ITYPES.OtpTokenRepository)
  .to(OtpTokenRepository)
  .inSingletonScope(); // Thêm binding
userContainer.bind(ITYPES.Datasource).toConstantValue(AppDataSource);

// Import service
userContainer
  .bind<IWalletService<any>>(SERVICE_TYPES.Wallet)
  .toConstantValue(walletService);
userContainer
  .bind<ICategoryService<any>>(SERVICE_TYPES.Category)
  .toConstantValue(categoryService);

const userController = userContainer.get<IUserController<any>>(
  ITYPES.Controller
);
const userService = userContainer.get<IUserService<any>>(ITYPES.Service);
const userRepository = userContainer.get<IUserRepository<User>>(
  ITYPES.Repository
);

export { userController, userService, userRepository, userContainer }; // Export userContainer nếu cần merge
